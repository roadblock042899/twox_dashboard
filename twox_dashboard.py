...
st.markdown("Track the performance, cap exposure, and risk metrics of the TWOX ETF (Accelerated Return).")

# --- INPUT SECTION ---
api_key = st.text_input("Enter your Alpha Vantage API Key:", type="password")
ivv_start_price = st.number_input("IVV Starting Price on April 1", value=505.00)
cap_level = st.slider("Strategy Cap (%)", min_value=5.0, max_value=25.0, value=15.0)
...
if api_key:
    ...
    if ivv_current_price:
        ...
        percent_to_cap = max(0, ((CAP_NAV - nav) / (CAP_NAV - START_NAV)) * 100)
